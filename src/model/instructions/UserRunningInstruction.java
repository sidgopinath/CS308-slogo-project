...
public class UserRunningInstruction extends Instruction{
	...
	public double execute() {
		double returnVal = 0;
		Instruction userCommand = myEnvironment.myUserInstructionMap.get(myInstructionType);
		// add all variables given, assigned to proper names
		for(int i = 0; i<myDependencies.get(0).myDependencies.size(); i++){
			myEnvironment.myVariableMap.put(userCommand.myDependencies.get(1).myDependencies.get(i).getName(), myDependencies.get(0).myDependencies.get(i).execute());
