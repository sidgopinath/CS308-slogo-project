...
public class Drawer {
    ...
    private int myIndex;
    private static final int WINDOW_CONSTANT = 2;
    private static final int FENCE_CONSTANT = 3;
    private int myWrapType = WINDOW_CONSTANT;
    ...
    public List<Polyline> draw (Map<Integer, TurtleView> turtles,
        ...
        while (it.hasNext()) {
            ...
            if (polar.getDistance() != 0) {
                ...
                if (newY < myYBounds[0] && myWrapType == FENCE_CONSTANT) {
                    moveY = myYBounds[0]-startY+15;
                }
                else if (newY > myYBounds[1] && myWrapType == FENCE_CONSTANT) {
                    moveY = myYBounds[1]-startY+15;
                }
                if (newX < myXBounds[0] && myWrapType == FENCE_CONSTANT) {
                    moveX = myXBounds[0]-startX+15;
                }
                else if (newX > myXBounds[1] && myWrapType == FENCE_CONSTANT) {
                    moveX = myXBounds[1]-startX+15;
                }
                if (newY < myYBounds[0] && myWrapType == WINDOW_CONSTANT) {
                    wrapY(1, turtle, polar, lines, 0, newY, startX, moveX, moveY,
                          turtleY, startY);
                }
                else if (newY > myYBounds[1] && myWrapType == WINDOW_CONSTANT) {
                    wrapY(1, turtle, polar, lines, 1, newY, startX, moveX, moveY,
                          turtleY, startY);
                }
                else if (newX < myXBounds[0] && myWrapType == WINDOW_CONSTANT) {
                    wrapX(0, turtle, polar, lines, 0, newX, startY, moveY, moveX,
                          turtleX, startX);
                }
                else if (newX > myXBounds[1] && myWrapType == WINDOW_CONSTANT) {
    ...
    // turtle.move(turtle.getTranslateX()+(lines.get(1).getPoints().get(2)-lines.get(0).getPoints().get(2)),
    // turtle.getTranslateY()+(lines.get(1).getPoints().get(3)-lines.get(0).getPoints().get(3)));
    // myIndex++;
    // }
    //
    ...
    // new Double[] { startX+(double)i/10*(endX-startX), startY+(double)i/10*(endY-startY),
    // startX+(double)(i+1)/10*(endX-startX), startY+(double)(i+1)/10*(endY-startY)});
    // System.out.println("adding line from "+
    // startX+(double)i/10*(endX-startX)+" "+startY+(double)i/10*(endY-startY)+
    // " to "+startX+(double)(i+1)/10*(endX-startX)+ " "+ startY+(double)(i+1)/10*(endY-startY));
    // myLines.add(pLine);
    // }
    // return myLines;
    // }
